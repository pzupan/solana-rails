  ...
  class TokenTransactionHelpers
     ...
     def self.create_associated_token_account(**args)
      ...
      if @ata_pubkey.blank?
        ...
        ata_pubkey = SolanaRails::TokenMethods.get_associated_token_address(
      ...
      transaction_instruction = TransactionInstruction.new(
        keys: [
      ...
      message = Message.from_transaction_instruction(transaction_instruction)

      puts "**** Message: #{message.to_json}"
      puts ''

      Transaction.from_message(message)
    end
